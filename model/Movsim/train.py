...
#
# SEQ_LEN = 24, BATCH_SIZE = 32, GENERATED_NUM = 10000
def generate_samples(model, batch_size, seq_len, generated_num, output_file):
    samples = []
    # logger.info("start gen "+output_file)
    for _ in range(int(generated_num / batch_size)):
        sample = model.sample(
            batch_size, seq_len).cpu().data.numpy().tolist()
        samples.extend(sample)
    # logger.info("Finish gen "+output_file)
    with open(output_file, 'w') as fout:
        for sample in samples:
            string = ' '.join([str(s) for s in sample])
            fout.write('%s\n' % string)

    # logger.info("finish write "+output_file)
...
def pretrain_model(
    ...
    for epoch in range(pre_epochs):
        loss = train_epoch(name, model, data_iter, criterion,
                           optimizer, batch_size, device)
        logger.info('Epoch [%d], loss: %f' % (epoch + 1, loss))
...
def train_epoch(name, model, data_iter, criterion, optimizer, batch_size, device=None):
    total_loss = 0.
    # global i, data, target
    if name == "G":
        # im = torch.LongTensor([i % 24 for i in range(47)])
        tim = torch.LongTensor([i % 12 for i in range(23)]).to(device)
