...
class Generator(nn.Module):
    ...
    def sample(self, batch_size, seq_len, x=None):  # 其实在这里sample需要的是对已知的数据进行采样，而不是固定的从零开始进行采样
        res = []
        flag = False  # whether sample from zero
        if x is None:
            flag = True
        if flag:  # 如果输入的数据是空的，那么flag设置为true们就代表开始从头采样
        ...
        if flag:
            ...
            for i in range(given_len):  # 这一步是对隐状态进行更新
                output, h, c = self.step(lis[i], h, c)
                samples.append(lis[i])
            x = output.multinomial(1)
            for i in range(given_len, seq_len):
                samples.append(x)  # 将上一步的点输入到隐状态中
                output, h, c = self.step(x, h, c)
                x = output.multinomial(1)
